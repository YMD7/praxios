...
Praxios は、Agent と一緒にタスクを進めるための Task Context Builder である。

ユーザーは Task Workspace で Agent に作業を依頼する。Agent は作業中に得た情報を
...
Praxios はこの流れを単純化する。ユーザーは Agent に「メールに返答があったので確認して
コンテキストを更新して」のように依頼する。Agent は元情報の場所を Source として保持し、
...
(2) ユーザーが Task Workspace で Agent に作業を依頼する

(3) Agent またはユーザーが Source を Task に紐づける

(4) Agent が Source を ingest して Context を更新する

(5) ユーザーが Sources と Context を見て、Agent の前提を確認できる
...
Source は、Agent やユーザーが参照した元情報である。
...
Context は Agent とユーザーが共有する正規の作業メモとして扱う。
...
Task Workspace と接続し、Agent が Source と Context を扱えるようにする。
...
(3) ユーザーが Agent に「Google Drive から業務委託契約のテンプレートを探して」と依頼する

(4) Agent が Drive ファイルを見つけ、Source として保存する

(5) Agent がテンプレート URL、概要、使いどころを Context に追加する

(6) Agent が契約相手、金額、発注日、支払条件など不足情報を質問する

(7) ユーザーが Agent に直接回答する

(8) Agent が回答を Context に反映する

(9) ユーザーが「メールに返答があったので確認してコンテキストを更新して」と依頼する

(10) Agent がメールスレッドを Source として保存し、確定した条件を Context に反映する
...
目指すのは、ユーザーが Agent に作業を依頼すると、その過程で得られた情報が
