...
- Workspace lint / health check は trust infrastructure として扱います。
- 初期実装は single-process CLI で始めてよいが、CLI に business logic を
  閉じ込めてはいけません。
- CLI、将来の UI、server、worker、agent tool は同じ application services と
  policy checks を呼べる構造にします。
