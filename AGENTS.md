...
- `AGENTS.md` は、常時読み込まれる薄い入口として保ってください。
- 詳細な設計判断、長い説明、個別仕様は `docs/`、`docs/adr/`、`spec/` に
  分離してください。
- 新しいルールを追加する前に、既存文書へのリンクや短い参照で足りるか確認
  してください。
- ADR が増えてきた場合は、個別 ADR をすべて列挙し続けず、
  `docs/adr/README.md` で読むべき ADR を整理してください。
